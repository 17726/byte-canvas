<template>
  <div class="node-highlight-controls">
    <a-space v-if="isTextNode" class="control-item highlightStyle" :style="highlightStyle">
      <a-color-picker v-model="textColor" size="small" @change="updateNodeColor" title="文本颜色" />
      <a-button
      ...
      <a-input-number
        ...
        v-model="fontSize"
        :style="{ width: '100px', backgroundColor: 'white', borderLeft: '1px solid black' }"
...
const toggleFontStrikethrough = () => {
  ...
  canvasStore.activeElements.forEach((element) => {
    if (element && element.id && element.props) {
      const textNode = element as TextState;
      canvasStore.updateNode(element.id, {
        props: {
          ...element.props,
          strikethrough: !textNode.props.strikethrough,
        },
      });
    }
  });
  ...
  canvasStore.activeElements.forEach((element) => {
    if (element && element.id && element.props) {
      ...
      canvasStore.updateNode(element.id, {
        props: {
          ...element.props,
          underline: !textNode.props.underline,
        },
      });
    }
  });
